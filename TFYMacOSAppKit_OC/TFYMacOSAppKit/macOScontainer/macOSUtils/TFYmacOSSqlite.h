...
@protocol TFYmacOSSqlite <NSObject>
@optional
/**
 自定义数据存储路径
 自定义数据库路径(目录即可)
 ...
 */
+ (NSString *)sqlitePasswordKey;

/// 自定义数据表主键名称
/**
 *** 返回自定义主键名称默认主键:_id ***
 */
+ (NSString *)sqliteMainkey;
...
@interface TFYmacOSSqlite : NSObject
/**
 * 说明: 存储模型数组到本地(事务方式)
...
/**当存储NSArray/NSDictionary属性并且里面是自定义模型对象时，模型对象必须实现NSSecureCoding协议，可以使用TFYmacOSSqlite库一行代码实现NSSecureCoding相关代码**/

///模型对象归档解归档实现
#define TFYmacOSSqliteCodingImplementation \
- (instancetype)initWithCoder:(NSCoder *)coder\
{\
    self = [super init];\
    if (self) {\
        [self sqliteDecode:coder];\
    }\
    return self;\
}\
- (void)encodeWithCoder:(NSCoder *)coder\
{\
    [self sqliteEncode:coder];\
}\
+ (BOOL)supportsSecureCoding {\
    return YES;\
}\
- (id)copyWithZone:(NSZone *)zone { return [self sqliteCopy]; }

@protocol TFYSqliteModelKeyValue <NSObject>
@optional
/// 模型类可自定义属性名称<json key名, 替换实际属性名>
+ (NSDictionary <NSString *, NSString *> *)sqliteModelReplacePropertyMapper;
/// 模型数组/字典元素对象可自定义类<替换实际属性名,实际类>
+ (NSDictionary <NSString *, Class> *)sqliteModelReplaceContainerElementClassMapper;
/// 模型类可自定义属性类型<替换实际属性名,实际类>
+ (NSDictionary <NSString *, Class> *)sqliteModelReplacePropertyClassMapper;

@end

@interface NSObject (TFYSqliteModel)<TFYSqliteModelKeyValue>
