...
static const NSString * TFYString            = @"TEXT";
static const NSString * TFYInt               = @"INTERGER";
static const NSString * TFYBoolean           = @"INTERGER";
static const NSString * TFYDouble            = @"DOUBLE";
static const NSString * TFYFloat             = @"DOUBLE";
static const NSString * TFYChar              = @"NVARCHAR";
static const NSString * TFYData              = @"BLOB";
static const NSString * TFYArray             = @"BLOB";
static const NSString * TFYDictionary        = @"BLOB";
static const NSString * TFYMutableArray      = @"BLOB";
static const NSString * TFYMutableDictionary = @"BLOB";
static const NSString * TFYDate              = @"DOUBLE";

typedef NS_OPTIONS(NSUInteger, TFYFieldType) {
    _String,
    _Int,
    _Boolean,
    _Double,
...
typedef NS_OPTIONS(NSUInteger, TFYQueryType) {
    _Where,
    _Order,
    _Limit,
    _WhereOrder,
...
@interface TFYPropertyInfo : NSObject

@property (nonatomic, assign, readonly) TFYFieldType type;
@property (nonatomic, copy, readonly) NSString * name;
@property (nonatomic, assign, readonly) SEL setter;
@property (nonatomic, assign, readonly) SEL getter;
@end

@implementation TFYPropertyInfo

- (TFYPropertyInfo *)initWithType:(TFYFieldType)type propertyName:(NSString *)property_name name:(NSString *)name{
    if (self = [super init]) {
...
+ (TFYmacOSSqlite *)shareInstance {
    static TFYmacOSSqlite * instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [TFYmacOSSqlite new];
    });
    return instance;
}
...
+ (NSString *)databaseCacheDirectory:(Class)model_class {
    if (model_class) {
        NSString * custom_path = [self exceSelector:@selector(sqlitePath) modelClass:model_class];
...
+ (TFYFieldType)parserFieldTypeWithAttr:(NSString *)attr {
    ...
    TFYFieldType field_type = _String;
...
+ (const NSString *)databaseFieldTypeWithType:(TFYFieldType)type {
    switch (type) {
        case _String:
            return TFYString;
        case _Int:
        ...
        case _Array:
            return TFYArray;
        case _Dictionary:
            return TFYDictionary;
        case _MutableArray:
            return TFYMutableArray;
        case _MutableDictionary:
            return TFYMutableDictionary;
        default:
            break;
    }
    return TFYString;
...
+ (NSDictionary *)parserSubModelObjectFieldsWithModelClass:(Class)model_class propertyName:(NSString *)main_property_name complete:(void(^)(NSString * key, TFYPropertyInfo * property_object))complete {
    ...
    SEL selector = @selector(ignorePropertys);
    ...
    for (int i = 0; i < property_count; i++) {
        ...
        TFYPropertyInfo * property_info = nil;
        if (property_attributes_list.count == 1) {
            // base type
            TFYFieldType type = [self parserFieldTypeWithAttr:property_attributes_list[0]];
            property_info = [[TFYPropertyInfo alloc] initWithType:type propertyName:property_name_string name:name];
        }else {
            // refernece type
            Class class_type = NSClassFromString(property_attributes_list[1]);
            if (class_type == [NSNumber class]) {
                property_info = [[TFYPropertyInfo alloc] initWithType:_Number propertyName:property_name_string name:name];
            }else if (class_type == [NSString class]) {
                property_info = [[TFYPropertyInfo alloc] initWithType:_String propertyName:property_name_string name:name];
            }else if (class_type == [NSData class]) {
                property_info = [[TFYPropertyInfo alloc] initWithType:_Data propertyName:property_name_string name:name];
            }else if (class_type == [NSArray class]) {
                property_info = [[TFYPropertyInfo alloc] initWithType:_Array propertyName:property_name_string name:name];
            }else if (class_type == [NSDictionary class]) {
                property_info = [[TFYPropertyInfo alloc] initWithType:_Dictionary propertyName:property_name_string name:name];
            }else if (class_type == [NSDate class]) {
                property_info = [[TFYPropertyInfo alloc] initWithType:_Date propertyName:property_name_string name:name];
            }else if (class_type == [NSMutableArray class]){
                property_info = [[TFYPropertyInfo alloc] initWithType:_MutableArray propertyName:property_name_string name:name];
            }else if (class_type == [NSMutableDictionary class]){
                property_info = [[TFYPropertyInfo alloc] initWithType:_MutableDictionary propertyName:property_name_string name:name];
                ...
                if (need_dictionary_save) {
                    [self parserSubModelObjectFieldsWithModelClass:class_type propertyName:name complete:^(NSString * key, TFYPropertyInfo *property_object) {
...
+ (void)updateTableFieldWithModel:(Class)model_class newVersion:(NSString *)newVersion localModelName:(NSString *)local_model_name {
    @autoreleasepool {
        ...
        if (sqlite3_open([database_cache_path UTF8String], &_tfy_database) == SQLITE_OK) {
            ...
            [new_model_info enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, TFYPropertyInfo * obj, BOOL * _Nonnull stop) {
...
+ (NSString *)getMainKeyWithClass:(Class)model_class {
    NSString * main_key = [self exceSelector:@selector(sqliteMainkey) modelClass:model_class];
...
#ifdef SQLITE_HAS_CODEC
    NSString * psw_key = [self exceSelector:@selector(sqlitePasswordKey) modelClass:model_class];
...
+ (NSString *)getSqlitePath:(Class)model_class {
    SEL selector = @selector(otherSqlitePath);
    if ([model_class respondsToSelector:selector]) {
...
+ (NSString *)autoHandleOldSqlite:(Class)model_class {
    ...
    if (sqlite_path && sqlite_path.length > 0) {
        ...
        NSString * version = [self exceSelector:@selector(sqliteVersion) modelClass:model_class];
...
+ (BOOL)openTable:(Class)model_class {
    ...
    SEL VERSION = @selector(sqliteVersion);
...
+ (BOOL)createTable:(Class)model_class {
    ...
    if (field_dictionary.count > 0) {
        ...
        [field_dictionary enumerateKeysAndObjectsUsingBlock:^(NSString * field, TFYPropertyInfo * property_info, BOOL * _Nonnull stop) {
...
+ (BOOL)commonInsert:(id)model_object {
    ...
    [field_array enumerateObjectsUsingBlock:^(NSString * field, NSUInteger idx, BOOL * _Nonnull stop) {
        TFYPropertyInfo * property_info = field_dictionary[field];
        ...
        [field_array enumerateObjectsUsingBlock:^(NSString *  _Nonnull field, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            switch (property_info.type) {
                ...
                case _Number:
                {
                    ...
                    sqlite3_bind_double(pp_stmt, index, number.doubleValue);
                }
                    break;
                case _Int:
                ...
                {
                    NSNumber *number = (NSNumber*)value;
                    sqlite3_bind_int(pp_stmt, index, number.intValue);
                }
                ...
                {
                    NSNumber *number = (NSNumber*)value;
                    sqlite3_bind_double(pp_stmt, index, number.floatValue);
                }
...
+ (NSArray *)commonQuery:(Class)model_class conditions:(NSArray *)conditions queryType:(TFYQueryType)query_type {
    NSString * table_name = [self getTableName:model_class];
    ...
    if (sqlite3_prepare_v2(_tfy_database, [sql UTF8String], -1, &pp_stmt, nil) == SQLITE_OK) {
        ...
        while (sqlite3_step(pp_stmt) == SQLITE_ROW) {
            ...
            for (int column = 1; column < colum_count; column++) {
                ...
                TFYPropertyInfo * property_info = field_dictionary[field_name];
...
+ (NSArray *)startQuery:(Class)model_class conditions:(NSArray *)conditions queryType:(TFYQueryType)query_type {
...
+ (NSArray *)queryModel:(Class)model_class conditions:(NSArray *)conditions queryType:(TFYQueryType)query_type {
...
typedef NS_OPTIONS(NSUInteger, TFYSqliteTYPE) {
    _SqliteArray = 1 << 0,
...
@interface TFYSqliteModelPropertyInfo : NSObject {
@public
    Class class;
    TFYSqliteTYPE type;
    SEL setter;
...
@end
@implementation TFYSqliteModelPropertyInfo
...
@implementation NSObject (TFYSqliteModel)

#pragma mark - 枚举类属性列表 -

+ (void)sqliteEnumeratePropertyNameUsingBlock:(void (NS_NOESCAPE ^)(NSString * propertyName, NSUInteger index, BOOL * stop))block {
...
+ (void)sqliteEnumeratePropertyAttributesUsingBlock:(void (NS_NOESCAPE ^)(NSString * propertyName,objc_property_t property, NSUInteger index, BOOL * stop))block {
    unsigned int propertyCount = 0;
    BOOL stop = NO;
    objc_property_t * properties = class_copyPropertyList(self, &propertyCount);
    for (unsigned int i = 0; i < propertyCount; i++) {
...
- (void)copySuperObject:(id)newSelf {
    ...
    if (superClass != nil &&
        ...
        [superClass sqliteEnumeratePropertyNameUsingBlock:^(NSString *propertyName, NSUInteger index, BOOL *stop) {
...
- (id)sqliteCopy {
    ...
    [self.class sqliteEnumeratePropertyAttributesUsingBlock:^(NSString *propertyName, objc_property_t property, NSUInteger index, BOOL *stop) {
        NSDictionary <NSString *, TFYSqliteModelPropertyInfo *> * propertyInfoMap = [self.class getModelPropertyDictionary];
        TFYSqliteModelPropertyInfo * propertyInfo = nil;
        if (propertyInfoMap != nil) {
            propertyInfo = propertyInfoMap[propertyName];
        }
        if (propertyInfo == nil) {
            propertyInfo = [TFYSqliteModelPropertyInfo new];
        ...
        if ([newSelf respondsToSelector:propertyInfo->setter]) {
            ...
            switch (propertyInfo->type) {
                ...
                case _SqliteFloat: {
                    NSNumber *number = (NSNumber*)value;
                    ((void (*)(id, SEL, float))(void *) objc_msgSend)((id)newSelf, propertyInfo->setter, [number floatValue]);
                }
                    break;
                case _SqliteDouble: {
                    NSNumber *number = (NSNumber*)value;
                    ((void (*)(id, SEL, double))(void *) objc_msgSend)((id)newSelf, propertyInfo->setter, [number doubleValue]);
                }
                    break;
                case _SqliteBoolean:{
                ...
                case _SqliteInteger:{
                    NSNumber *number = (NSNumber*)value;
                    ((void (*)(id, SEL, NSInteger))(void *) objc_msgSend)((id)newSelf, propertyInfo->setter, [number integerValue]);
                }
                    break;
...
- (void)sqliteEncode:(NSCoder *)aCoder {
    Class superClass = class_getSuperclass(self.class);
    if (superClass != nil &&
        superClass != [NSObject class]) {
        NSObject * superObject = superClass.new;
        ...
        [superObject sqliteEncode:aCoder];
    }
    [self.class sqliteEnumeratePropertyNameUsingBlock:^(NSString *propertyName, NSUInteger index, BOOL *stop) {
        id value = [self valueForKey:propertyName];
        if (value != nil) {
...
- (void)sqliteDecode:(NSCoder *)aDecoder {
    Class superClass = class_getSuperclass(self.class);
    if (superClass != nil &&
        superClass != [NSObject class]) {
        NSObject * superObject = superClass.new;
        [superObject sqliteDecode:aDecoder];
        [superClass sqliteEnumeratePropertyNameUsingBlock:^(NSString *propertyName, NSUInteger index, BOOL *stop) {
            ...
            if ([value isKindOfClass:[NSNumber class]]) {
                ...
                if ([self respondsToSelector:propertyInfo->setter]) {
                    switch (propertyInfo->type) {
                        ...
                        {
                            NSNumber *number = (NSNumber*)value;
                            ((void (*)(id, SEL, float))(void *) objc_msgSend)((id)self, propertyInfo->setter, [number floatValue]);
                        }
                        ...
                        {
                            NSNumber *number = (NSNumber*)value;
                            ((void (*)(id, SEL, double))(void *) objc_msgSend)((id)self, propertyInfo->setter, [number doubleValue]);
                        }
                        ...
                        {
                            NSNumber *number = (NSNumber*)value;
                            ((void (*)(id, SEL, NSInteger))(void *) objc_msgSend)((id)self, propertyInfo->setter, [number integerValue]);
                        }
...
+ (id)sqliteModelWithJson:(id)json {
    if (json) {
        if ([json isKindOfClass:[NSData class]]) {
            ...
            return [self sqliteModelWithJson:jsonData];
        }
    }
    return nil;
}

+ (id)sqliteModelWithJson:(id)json keyPath:(NSString *)keyPath {
    if (json) {
        if (keyPath != nil && keyPath.length > 0) {
            ...
            return [self sqliteModelWithJson:json];
        }
    }
    return nil;
}
...
- (NSDictionary *)sqliteDictionary {
    ...
    if (superClass != nil &&
        ...
        [jsonDictionary setDictionary:[superObject sqliteDictionary]];
    }
    NSDictionary <NSString *, TFYSqliteModelPropertyInfo *> * propertyInfoMap = [self.class getModelPropertyDictionary];
    [self.class sqliteEnumeratePropertyAttributesUsingBlock:^(NSString *propertyName, objc_property_t property, NSUInteger index, BOOL *stop) {
        TFYSqliteModelPropertyInfo * propertyInfo = nil;
        ...
        if (propertyInfo) {
            ...
            switch (propertyInfo->type) {
                ...
                case _SqliteModel: {
                    id value = ((id (*)(id, SEL))(void *) objc_msgSend)((id)self, propertyInfo->getter);
                    [jsonDictionary setObject:[value sqliteDictionary] forKey:propertyName];
                }
                    break;
                case _SqliteArray: {
                    id value = ((id (*)(id, SEL))(void *) objc_msgSend)((id)self, propertyInfo->getter);
...
- (id)parserDictionaryEngine:(NSDictionary *)value {
    ...
    for (NSString * key in allKey) {
        ...
        if ([subValue isKindOfClass:[NSString class]] ||
            ...
            [subJsonDictionary setObject:[subValue sqliteDictionary] forKey:key];
        }
    }
...
- (id)parserArrayEngine:(NSArray *)value {
    ...
    for (id subValue in value) {
        if ([subValue isKindOfClass:[NSString class]] ||
            ...
            [subJsonArray addObject:[subValue sqliteDictionary]];
        }
    }
    return subJsonArray;
}
...
static const char  TFYSqliteModelPropertyInfokey = '\0';
static const char  TFYSqliteReplaceKeyValue = '\0';
static const char  TFYSqliteReplacePropertyClass = '\0';
static const char  TFYSqliteReplaceContainerElementClass = '\0';

+ (NSDictionary <NSString *, Class> *)getContainerElementClassMapper {
    return objc_getAssociatedObject(self, &TFYSqliteReplaceContainerElementClass);
}

+ (void)setContainerElementClassMapper:(NSDictionary <NSString *, Class> *)mapper {
    objc_setAssociatedObject(self, &TFYSqliteReplaceContainerElementClass, mapper, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

+ (NSDictionary <NSString *, Class> *)getModelPropertyClassMapper {
    return objc_getAssociatedObject(self, &TFYSqliteReplacePropertyClass);
}

+ (void)setModelPropertyClassMapper:(NSDictionary <NSString *, Class> *)mapper {
    objc_setAssociatedObject(self, &TFYSqliteReplacePropertyClass, mapper, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

+ (NSDictionary <NSString *, NSString *> *)getModelReplacePropertyMapper {
    return objc_getAssociatedObject(self, &TFYSqliteReplaceKeyValue);
}

+ (void)setModelReplacePropertyMapper:(NSDictionary *)mapper {
    objc_setAssociatedObject(self, &TFYSqliteReplaceKeyValue, mapper, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

+ (NSDictionary <NSString *, TFYSqliteModelPropertyInfo *>*)getModelPropertyDictionary {
    return objc_getAssociatedObject(self, &TFYSqliteModelPropertyInfokey);
}

+ (TFYSqliteModelPropertyInfo *)getPropertyInfo:(NSString *)property {
    NSDictionary * propertyInfo = objc_getAssociatedObject(self, &TFYSqliteModelPropertyInfokey);
    return propertyInfo != nil ? propertyInfo[property] : nil;
}

+ (void)setModelInfo:(TFYSqliteModelPropertyInfo *)modelInfo property:(NSString *)property {
    NSMutableDictionary * propertyInfo = objc_getAssociatedObject(self, &TFYSqliteModelPropertyInfokey);
    if (propertyInfo == nil) {
        propertyInfo = [NSMutableDictionary dictionary];
        objc_setAssociatedObject(self, &TFYSqliteModelPropertyInfokey, propertyInfo, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
...
+ (TFYSqliteTYPE)parserTypeWithAttr:(NSString *)attr {
    NSArray * sub_attrs = [attr componentsSeparatedByString:@","];
    NSString * first_sub_attr = sub_attrs.firstObject;
    first_sub_attr = [first_sub_attr substringFromIndex:1];
    TFYSqliteTYPE attr_type = _SqliteNull;
    const char type = *[first_sub_attr UTF8String];
    switch (type) {
        case 'B':
...
+ (TFYSqliteModelPropertyInfo *)classExistProperty:(NSString *)property withObject:(NSObject *)object valueClass:(Class)valueClass {
    TFYSqliteModelPropertyInfo * propertyInfo = nil;
    objc_property_t property_t = class_getProperty(object.class, [property UTF8String]);
    if (property_t != NULL) {
    ...
    if(object) {
        if([object isKindOfClass:[NSDictionary class]]) {
            ...
            if (replacePropertyNameMap == nil &&
                [class respondsToSelector:@selector(sqliteModelReplacePropertyMapper)]) {
                replacePropertyNameMap = [class sqliteModelReplacePropertyMapper];
            ...
            if ([class isSubclassOfClass:[NSDictionary class]]) {
                ...
                [dictionary enumerateKeysAndObjectsUsingBlock:^(NSString * key, id  _Nonnull obj, BOOL * _Nonnull stop) {
                    ...
                    TFYSqliteModelPropertyInfo * propertyInfo = [class getPropertyInfo:actualProperty];
                    if (propertyInfo == nil || (propertyInfo != nil && propertyInfo->type == _SqliteUnknown)) {
                        if (replacePropertyClassMap) {
                            ...
                            if ([class respondsToSelector:@selector(sqliteModelReplacePropertyClassMapper)]) {
                                [class setModelPropertyClassMapper:[class sqliteModelReplacePropertyClassMapper]];
                            }
                    ...
                    switch (propertyInfo->type) {
                        case _SqliteArray:
                            if(![subObject isKindOfClass:[NSNull class]]){
                                ...
                                }else if ([class respondsToSelector:@selector(sqliteModelReplaceContainerElementClassMapper)]) {
                                    replaceContainerElementClassMap = [class sqliteModelReplaceContainerElementClassMapper];
                                    subModelClass = replaceContainerElementClassMap[actualProperty];
                        ...
                        case _SqliteInteger:
                        {
                            NSNumber *number = (NSNumber*)subObject;
                            ((void (*)(id, SEL, NSInteger))(void *) objc_msgSend)((id)modelObject, propertyInfo->setter, [number integerValue]);
                        }
                        ...
                        {
                            NSNumber *number = (NSNumber*)subObject;
                            ((void (*)(id, SEL, float))(void *) objc_msgSend)((id)modelObject, propertyInfo->setter, [number floatValue]);
                        }
                        ...
                        {
                            NSNumber *number = (NSNumber*)subObject;
                            ((void (*)(id, SEL, double))(void *) objc_msgSend)((id)modelObject, propertyInfo->setter, [number doubleValue]);
                        }
